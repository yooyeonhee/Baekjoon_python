...
p_result = []
time = 0
n_t = now[0] * 3600 + now[1] * 60 + now[2]
s_t = start[0] * 3600 + start[1] * 60 + start[2]
if n_t <= s_t:
    time = s_t - n_t
else:
    time = 86400 - (n_t - s_t)
result.append(time // 3600)
result.append((time % 3600) // 60)
result.append((time % 3600) % 60)
for i in result:
    if i < 10:
        p_result.append(str("0%d" % i))
    else:
        p_result.append(str(i))
print(':'.join(map(str, p_result)))
